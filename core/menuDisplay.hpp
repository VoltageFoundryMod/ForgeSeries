...
// ── Layout constants ─────────────────────────────────────────
// The header occupies rows 1–8, so the first row starts at 12. With 9px rows
// that puts a six-row page at y = 12,21,30,39,48,57 — the last row's glyphs end
// on row 63, exactly filling the 64px screen. Do not raise MD_START_Y without
// checking the longest page (SCALES and PITCH both have six rows): at 20 the
// sixth row lands at y=65 and is silently clipped.
static const int MD_ROW_H = 9;    // pixels per row
// Overridable: ClockForge's generic group pages sit at 20. A macro rather than
// a const so an app can set it before including this header.
#ifndef MD_START_Y
#define MD_START_Y 12 // first row Y (below header)
#endif
...
// firmware is a plain list, so this handles all of them.
