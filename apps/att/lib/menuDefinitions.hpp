...
enum RowStyle : uint8_t {
    ...
    ROW_ACTION, // label only, no value (save, load, reseed)
...
struct MenuItem {
    ...
    // Adjusting this item visibly changes the orbit, so hand the screen to the
    // plot while it is being turned — see the live-view section of
    // menuHandlers.hpp.
    //
...
// The plot page is group 0 and is the module's home screen.
