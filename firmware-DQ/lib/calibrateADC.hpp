...
// Helper function to average ADC readings
uint32_t AverageADCRead(int pin, int samples = 10) {
    uint32_t sum = 0;
...
void CalibrateDAC(Adafruit_SSD1306 &display) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        display.display();
        for (int j = 2; j < 2 + NUM_CHANNELS; j++) {
...
void CalibrateADC(Adafruit_SSD1306 &display) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        calib.calibrationValues[0][i] = AverageADCRead(CV_1_IN_PIN);
        calib.calibrationValues[1][i] = AverageADCRead(CV_2_IN_PIN);
