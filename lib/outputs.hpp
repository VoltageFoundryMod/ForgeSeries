...
class Output {
  public:
    ...
    void SetEuclideanParams(EuclideanParams params) {
        _euclideanParams = params;
        // Clamp to valid ranges (defensive against corrupt presets) and rebuild
        // the pattern so a loaded preset takes effect immediately.
        _euclideanParams.steps = constrain(_euclideanParams.steps, 1, MaxEuclideanSteps);
        _euclideanParams.triggers = constrain(_euclideanParams.triggers, 1, _euclideanParams.steps);
        _euclideanParams.rotation = constrain(_euclideanParams.rotation, 0, _euclideanParams.steps - 1);
        _euclideanParams.pad = constrain(_euclideanParams.pad, 0, MaxEuclideanSteps - _euclideanParams.steps);
    ...
    void SetQuantizerParams(QuantizerParams params) {
        _quantizerParams = params;
        // Clamp to valid ranges (defensive against corrupt presets) and rebuild
        // the note buffer so a loaded preset's scale/root takes effect now —
        // not only after the user next nudges a quantizer parameter.
        _quantizerParams.scaleIndex = constrain(_quantizerParams.scaleIndex, 0, numScales - 1);
        _quantizerParams.noteIndex = constrain(_quantizerParams.noteIndex, 0, 11);
        _quantizerParams.octaveShift = constrain(_quantizerParams.octaveShift, 0, 6);
        _quantizerParams.channelSensitivity = constrain(_quantizerParams.channelSensitivity, 0, 8);
        SetupQuantizer();
    }
    void SetupQuantizer();
    ...
    void SetQuantizerScaleIndex(int index) {
        _quantizerParams.scaleIndex = constrain(index, 0, numScales - 1);
        SetupQuantizer();
    }
