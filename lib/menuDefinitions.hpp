...
enum RowStyle : uint8_t {
    ...
    ROW_ACTION, // label only, no value (save, load, load scale)
    ROW_HIDDEN, // item belongs to a page but has no dedicated row (keyboard keys)
};
...
// The keyboard page is group 0 and is the module's home screen.
